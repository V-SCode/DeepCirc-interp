"""Figure S05 — Panel A v2: blue→red palette variant.

Same data + layout as build_panel_a.py; "bad" tiers (toxic_extreme,
toxic_tier2) swapped from near-black / MID_GRAY to deep-red /
tier-2-red. Background toxic-region tint also red (low alpha).
Compare against canonical panel_a.pdf for blue-to-red vs blue-to-grey.

Outputs: panels/vector/panel_a_v2.{pdf,svg}
         panels/raster/panel_a_v2.png

Run:
    python \\
        figures/setFinal/figS14/scripts/build_panel_a_v2.py
"""
...
PART_CLASS_COLORS = {
    "toxic_extreme":  BAD_DEEP,    # deep red — strongest "toxic" emphasis
    "toxic_tier2":    BAD_TIER2,   # tier-2 red
...
def build_panel() -> None:
    ...
    if toxic_idxs:
        # v2: toxic-region tint goes back to a low-alpha red wash so the
        # "bad zone" reads chromatically as well as in the dot emphasis.
    ...
    fig.text(0.58, 0.975, "Per part growth score probability enrichment",
              ha="center", va="top",
    ...
    out_base = PANELS_VEC / "panel_a_v2"
    written = save_panel(fig, out_base, dpi=600, close=True)
    print("Panel A v2 written:")
